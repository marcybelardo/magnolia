...
int main(void)
{
	...
	for (;;) {
		...
		int newsockfd =
		    accept(conn.sockfd, (struct sockaddr *)conn.host_addr,
			   (socklen_t *)&conn.host_addrlen);
		if (newsockfd < 0) {
		...
		// Get client address
		int sockn =
		    getsockname(newsockfd, (struct sockaddr *)conn.client_addr,
				(socklen_t *)&conn.client_addrlen);
		if (sockn < 0) {
		...
		printf("[%s:%u] %d %f %s\n",
		       ...
		       ntohs(conn.client_addr->sin_port), request.method,
		       request.HTTP_version, request.uri);
		...
		if (combine_resp) {
			...
			memcpy(combine_resp + head_len, resp.content,
			       cont_len + 1);
		}

		// Write to the socket
		int valwrite =
		    write(newsockfd, combine_resp, head_len + cont_len + 1);
		if (valwrite < 0) {
